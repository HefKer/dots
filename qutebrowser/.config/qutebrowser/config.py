...
def file_picker(chooser_flag):
    wezterm = shutil.which("wezterm")
    shell = shutil.which("fish") or shutil.which("bash")
    ranger = shutil.which("ranger")

    if not all([wezterm, shell, ranger]):
        return None
    ...
    return [
        ...
        shell,
        "-c",
        f"{ranger} {chooser_flag} {{}}",
    ]


single = file_picker("--choosefile")
multiple = file_picker("--choosefiles")
